...
function numberToArabicWords(amount: number): string {
  ...
  const r = total % 1_000;
  const parts: string[] = [];
  if (b === 1) parts.push("مليار"); else if (b === 2) parts.push("ملياران"); else if (b > 2) parts.push(threeDigits(b) + " مليارات");
  if (m === 1) parts.push("مليون"); else if (m === 2) parts.push("مليونان"); else if (m > 2) parts.push(threeDigits(m) + " ملايين");
  if (k === 1) parts.push("ألف"); else if (k === 2) parts.push("ألفان"); else if (k > 2 && k < 11) parts.push(threeDigits(k) + " آلاف"); else if (k >= 11) parts.push(threeDigits(k) + " ألف");
...
export default function ReceiptPrint() {
  ...
  const [showStamp, setShowStamp] = useState<boolean>(() => {
    try { return localStorage.getItem("receipt_show_stamp") !== "false"; }
    catch { return true; }
  });

  function toggleStamp(val: boolean) {
    setShowStamp(val);
    try { localStorage.setItem("receipt_show_stamp", val ? "true" : "false"); } catch {}
  }

  useEffect(() => {
    if (!receipt) return;
    const prev = document.title;
    document.title = `${receipt.receiptNumber} - ${receipt.clientName}`;
    return () => { document.title = prev; };
  }, [receipt]);

  if (isLoading) return <div className="p-8 text-center">{isAR ? "جارٍ تحميل السند..." : "Loading receipt..."}</div>;
  if (!receipt) return <div className="p-8 text-center text-red-600">{isAR ? "السند غير موجود" : "Receipt not found"}</div>;
  ...
  return (
    <div className="min-h-screen bg-gray-100 print:bg-white" dir="rtl">
      ...
      <div className="print:hidden flex gap-3 p-4 max-w-2xl mx-auto flex-wrap" dir={isAR ? "rtl" : "ltr"}>
        ...
        >
          <Printer className="w-4 h-4" /> {isAR ? "طباعة السند" : "Print Receipt"}
        </button>
      ...
      >
        ...
        {settings.showWatermark && (
          ...
          >
            ...
            <div className="text-center leading-tight">
              <div className="text-3xl font-black text-blue-800" style={{ fontFamily: "'Cairo', sans-serif" }}>{settings.nameAr}</div>
              <div className="text-xl font-black text-blue-800 mt-1">{settings.nameEn.split(" ").slice(0, 3).join(" ")}</div>
            </div>
          </div>
        )}
        ...
        <div className="border-b-4 border-double border-gray-800 pb-1 pt-1 px-5" style={{ position: "relative", zIndex: 1 }}>
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <div className="text-sm font-black text-gray-900 leading-tight">{settings.nameAr.split(" ").slice(0, 2).join(" ")}</div>
            ...
            <div className="text-left">
              <div className="text-sm font-black text-gray-900 leading-tight">{settings.nameEn.split(" ").slice(0, 3).join(" ").toUpperCase()}</div>
              ...
              <div className="text-[10px] text-gray-500">Tel: {printPhone} · {settings.address}</div>
            </div>
          </div>
          {/* Receipt number + barcode row */}
          ...
          <table className="w-full text-xs border-collapse border border-gray-300">
            <tbody>
              ...
              <tr className="border-b border-gray-200">
                <td className="px-3 py-1.5 font-bold text-gray-700 text-right bg-gray-50 w-32 border-l border-gray-200">العميل</td>
                <td className="px-3 py-1.5 font-semibold text-gray-900 text-center border-l border-gray-200">{receipt.clientName}</td>
                <td className="px-3 py-1.5 font-bold text-gray-400 text-left bg-gray-50 w-32 tracking-wide">Customer</td>
              </tr>
              {/* طريقة الدفع */}
              <tr className="border-b border-gray-200">
                <td className="px-3 py-1.5 font-bold text-gray-700 text-right bg-gray-50 border-l border-gray-200">طريقة الدفع</td>
                <td className="px-3 py-1.5 font-semibold text-gray-900 text-center border-l border-gray-200">{PAYMENT_METHOD_LABELS[receipt.paymentMethod] ?? receipt.paymentMethod}</td>
                <td className="px-3 py-1.5 font-bold text-gray-400 text-left bg-gray-50 tracking-wide">Payment Method</td>
              </tr>
              {/* رقم الفاتورة */}
              <tr className="border-b border-gray-200">
                <td className="px-3 py-1.5 font-bold text-gray-700 text-right bg-gray-50 border-l border-gray-200">رقم الفاتورة</td>
                <td className="px-3 py-1.5 font-semibold text-gray-900 text-center font-mono border-l border-gray-200">{receipt.invoiceNumber ?? "—"}</td>
                <td className="px-3 py-1.5 font-bold text-gray-400 text-left bg-gray-50 tracking-wide">Invoice No</td>
              </tr>
              {/* التاريخ */}
              <tr>
        ...
        <div className="grid grid-cols-2 gap-8 px-10 pb-3 pt-4 border-t border-gray-300" style={{ position: "relative", zIndex: 3 }}>
          {/* Stamp — absolute centred, no effect on layout */}
