...
router.post("/auth/login", async (req, res) => {
  ...
  if (user.pendingApproval) {
    return res.status(403).json({ message: "حسابك في انتظار تفعيل من قبل المدير — الرجاء المحاولة لاحقاً", pendingApproval: true });
  }
  ...
  // Generate and store OTP (always required)
  ...
  // Try sending via email and/or WhatsApp
  let sent = false;

  if (user.email) {
    try { sent = await sendOTPEmail(user.email, code, user.displayName); }
  ...
  if (user.phone && user.whatsappApiKey) {
    try {
      ...
      if (waSent) sent = true;
    } catch (err) { console.error("[OTP] WhatsApp error:", err); }
  }
  ...
  return res.json({
    ...
    // Show code on screen only if nothing was actually sent
